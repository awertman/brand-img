class Post < ActiveRecord::Base
  ...
  def self.get_posts_and_sort_by_tag_matches brand, filters, base_tags
    ...
    posts = Post.where(brand_id: brand).joins(:tags).where(tags: {description: filters}).select("post_id, count(tags)").group(:post_id).having("count(tags) = ?", filters.length)
